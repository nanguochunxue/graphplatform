...
import static com.haizhi.graph.plugins.flume.source.taildir.TaildirSourceConfigurationConstants.*;
import static org.junit.Assert.*;

public class TaildirSourceTest {
    ...
    public void testRegexFileNameFilteringEndToEnd() throws IOException {
        File dir1 = new File(tmpDir, "hello-a");
        dir1.mkdir();
        File f1 = new File(tmpDir.getAbsolutePath() + "/hello-a","a.log");
        File f2 = new File(tmpDir.getAbsolutePath() + "/hello-a", "a.log.1");

        File dir2 = new File(tmpDir, "hello-b");
        dir2.mkdir();
        File f3 = new File(tmpDir.getAbsolutePath() + "/hello-b", "b.log");
        File f4 = new File(tmpDir.getAbsolutePath() + "/hello-b", "c.log.yyyy-MM-01");
        File f5 = new File(tmpDir.getAbsolutePath() + "/hello-b", "c.log.yyyy-MM-02");
        ...
        context.put(FILE_GROUPS_PREFIX + "ab", tmpDir.getAbsolutePath() + "/hello-.*/[ab].log");
        // Tail files that starts with c.log
        context.put(FILE_GROUPS_PREFIX + "c", tmpDir.getAbsolutePath() + "/hello-.*/c.log.*");
        context.put(FILE_GROUPS_PREFIX + "ab.scanDir", tmpDir.getAbsolutePath());
        context.put(FILE_GROUPS_PREFIX + "c.scanDir", tmpDir.getAbsolutePath());
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (e != null) {
                out.add(TaildirEventReaderTest.bodyAsString(e));
            }
        }
        txn.commit();
        txn.close();
