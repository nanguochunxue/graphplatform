...
public class TaildirMatcher {
    ...
    private final File scanDirFile;
    ...
    TaildirMatcher(String fileGroup, String filePattern, boolean cachePatternMatching) {
        ...
        this.scanDirFile = f.getParentFile();
        ...
        // sanity check
        Preconditions.checkState(scanDirFile.exists(),
                "Directory does not exist: " + scanDirFile.getAbsolutePath());
    }

    TaildirMatcher(String fileGroup, FileFinder fileFinder, boolean cachePatternMatching) {
        // store whatever came from configuration
        this.fileGroup = fileGroup;
        this.filePattern = fileFinder.getFilePattern();
        this.cachePatternMatching = cachePatternMatching;

        // calculate final members
        File f = new File(filePattern);
        this.scanDirFile = new File(fileFinder.getScanDir());
        String regex = f.getName();
        final PathMatcher matcher = FS.getPathMatcher("regex:" + fileFinder.getFilePattern());
        this.fileFilter = new DirectoryStream.Filter<Path>() {
            @Override
            public boolean accept(Path entry) throws IOException {
        ...
        /**
         * Lists those files within the scanDirFile that match regex pattern passed in during object
         * instantiation. Designed for frequent periodic invocation
         * {@link org.apache.flume.source.PollableSourceRunner}.
         * <p></p>
         * Based on the modification of the scanDirFile this function may trigger cache recalculation by
         ...
         * Additionally if a constantly updated directory was configured as scanDirFile
         * then multiple changes to the scanDirFile may happen
    ...
    List<File> getMatchingFiles() {
        ...
        long currentParentDirMTime = scanDirFile.lastModified();
        List<File> result;
    ...
    /**
     * Provides the actual files within the scanDirFile which
    ...
    private List<File> getMatchingFilesNoCache() {
        ...
        try {
            result = Files.walk(Paths.get(scanDirFile.getPath())).filter(new Predicate<Path>() {
                @Override
                public boolean test(Path path) {
                    try {
                        return fileFilter.accept(path);
                    } catch (IOException e) {
                        logger.error("I/O exception occurred while filter path :" + scanDirFile.toPath(), e);
                        return false;
                    }
                }
            ...
            logger.error("I/O exception occurred while walking directory. " +
                    "Files already matched will be returned. " + scanDirFile.toPath(), e);
        }
        return result;
    }
