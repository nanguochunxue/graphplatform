...
public class TailFile {
    ...
    private String isCsvFile;
    private boolean isCsvFileBool;

    public TailFile(File file, Map<String, String> headers, long inode, long pos)
            throws IOException {
        this.raf = new RandomAccessFile(file, "r");

        // add to verify is a csv file, if so, cache the header to every line, used for data interceptor to assembly a json format
        isCsvFileBool = Boolean.valueOf(headers.get(InboundFlumeConstants.IS_CSV)) ^ file.getName().endsWith(".csv");
        isCsvFile = String.valueOf(isCsvFileBool);
        if (isCsvFileBool) {
            raf.seek(0);
            csvHeader = raf.readLine();
            ...
            logger.info("read file : " + file.getParent() + " , " + isCsvFile + " ; header is [" + csvHeader + "]");
        } else if (pos > 0) {
            raf.seek(pos);
            lineReadPos = pos;
        }
    ...
    private Event readEvent(boolean backoffWithoutNL, boolean addByteOffset) throws IOException {
        ...
        if (isCsvFileBool) {
            event.getHeaders().put(InboundFlumeConstants.CSV_HEADER, csvHeader);
            event.getHeaders().put(InboundFlumeConstants.IS_CSV, isCsvFile);
        }
