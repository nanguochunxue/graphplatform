...
export PRO_PATH="${curDir}"
export ModuleName="graph-tag-analytics"
...
case "$1" in
	start|stop|restart)
  		case "$2" in
  		    cmb|haizhi|haizhi-fi|haizhi-fic80|haizhi-tdh|test|uat)
  		        $1
  		        ;;
  		    *)
  		        echo $"Usage: $0 {start|stop|restart} {cmb|haizhi|haizhi-fi|haizhi-fic80|haizhi-tdh|test|uat}"
  		        exit 2
  		esac
		;;
	*)
