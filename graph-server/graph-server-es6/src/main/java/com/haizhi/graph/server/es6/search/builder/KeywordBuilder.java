...
public class KeywordBuilder {
    ...
    public static QueryBuilder get(EsQuery esQuery) {
        ...
        for (Map<String, Object> schemaQuery : query) {
            ...
            String operator = schemaQuery.get(EKeys.operator).toString();
            ...
            if (operator.equals(QType.MATCH.name())) {
                queryBuilder = QueryBuilders.matchPhraseQuery(field, keyword).slop(3).boost(boost);
            } else if (operator.equals(QType.EXISTS.name())) {
                queryBuilder = QueryBuilders.existsQuery(field).boost(boost);
            }
            if (Objects.nonNull(queryBuilder)) {
                keywordBuilder.should(queryBuilder);
            }
        }
        return keywordBuilder;
    }
